...
export default function Shop() {
  ...
  const getTypeDisplay = (type) => {
    ...
    return type.translated_name || type.name;
  };
  ...
  const loadProductTypes = async (categoryId) => {
    try {
      ...
      // Load all product types (product_types doesn't have category_id column)
      const { data, error } = await supabase
        .from('product_types')
        .select('*')
        .order('name', { ascending: true });
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (selectedType !== t('shop_page.all_categories')) {
      const typeObj = productTypes.find(t => t.name === selectedType || t.id == selectedType);
      const typeId = typeObj ? typeObj.id : selectedType;
  ...
  const getTypeName = (product) => {
    if (product.product_type?.translated_name) return product.product_type.translated_name;
    if (product.product_type?.name) return product.product_type.name;
    if (product.type?.translated_name) return product.type.translated_name;
    if (product.type?.name) return product.type.name;
    const type = productTypes.find(t => t.id === product.type_id);
    if (type) return type.translated_name || type.name;
    return t('shop_page.not_defined');
  };
